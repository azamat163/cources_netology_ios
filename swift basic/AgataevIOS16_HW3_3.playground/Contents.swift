...
// Создал пустой массив строк String
var cutleries: [String] = []


// 2 функции с условием

// функция добавления в массив
func addNewItemInArray(array: inout [String], newItem: String) -> [String] {
    array.append(newItem)
    return array
}

//Что произойдет, если попытаетесь удалить значение, которого нет в Хранилище? Напишите, что в этом случае вы можете изменить своей функции и внесите в нее изменения.
// В этом случае, получим ошибку - нам необходимо это проверить условием if let

// функция удаления из массива
func deleteItemInArray(array: inout [String], item: String) -> [String] {
    if let index = array.firstIndex(of: item) {
        array.remove(at: index)
    } else {
        print("Такого \(item) элемента нет в массиве")
    }
    return array
}
...
addNewItemInArray(array: &cutleries, newItem: "Kotlin")
addNewItemInArray(array: &cutleries, newItem: "Swift")
addNewItemInArray(array: &cutleries, newItem: "String")
deleteItemInArray(array: &cutleries, item: "Str")
deleteItemInArray(array: &cutleries, item: "Swift")
//: ## Задача №2
...
//print(userPoints)
func conversionPointToMoney(dict: [Int: Double], coef: Double) -> [Int: Int] {
    let results: [Int: Int] = dict.mapValues { value in  return Int(value * coef) } // есть closure mapValues которые как раз значение value можно преобразовать
    return results
}
print("dict : \(userPoints)")
print("convert dict : \(conversionPointToMoney(dict: userPoints, coef: 0.1))")
//: ## Задача №3 (*)
...
func calculateSumEvenOddValues(from array: [Int?]) -> (Int, Int) {
    var sumEven = 0 // счетчик суммы для четных значений
    var sumOdd = 0 // счетчик суммы для нечетных значений
    let toSet = Set(array) // убираем все дублирующие item в массиве
    for item in toSet {
        if let it = item {
            if it % 2 == 0 { // проверяем на четность
                sumEven += it
            } else {
                sumOdd += it
            }
    ...
    print("сумма четных: \(sumEven), нечетных: \(sumOdd)")
